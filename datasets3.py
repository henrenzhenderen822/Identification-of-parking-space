...
class Patchs(Dataset):
    ...
    def __init__(self, root, tf, mode):   # mode:模式（训练 or 测试）   tf:transform对图像采取变换
        ...
        self.images, self.labels = self.load_txt('datasets/PKLot_labels/all.txt')
...
train_datasets = Patchs('datasets/PKLot_patches', data_transforms['train'], mode='train')
test_datasets = Patchs('datasets/PKLot_patches', data_transforms['test'], mode='test')
