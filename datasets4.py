...
class Patchs(Dataset):
    ...
    def __init__(self, root, tf, mode):   # mode:模式（训练 or 测试）   tf:transform对图像采取变换
        ...
        self.images, self.labels = self.load_txt('datasets/mix_labels/mix.txt')
...
train_datasets = Patchs('datasets', data_transforms['train'], mode='train')
test_datasets = Patchs('datasets', data_transforms['test'], mode='test')
